...
//		ipb.show();
		...
		//bilder werden leer
		int[] pixelI= (int[])binary.getPixels();
		
		//Cast des Interger-Arrays in ein Byte-Array
		byte[] pixelB = new byte[pixelI.length];  
		  for(int i = 0; i < pixelI.length; i++){
		   pixelB[i] = (byte)pixelI[i];
		  }
...
//		impmb.show();
		...
		LUT lut1=LutLoader.openLut("C:/Program Files/ImageJ/luts/Green_Fire_Blue.lut");	
		ipmb.setLut(lut1);
